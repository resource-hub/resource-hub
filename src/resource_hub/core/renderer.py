...
from .utils import money_filter, normalize_fraction
...
class NumberedCanvas(Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        self.drawRightString(self._pagesize[0] - 20 * mm, 10 * mm, pgettext(
            "invoice", "Page %d of %d") % (self._pageNumber, page_count,))
        self.restoreState()
...
class BaseInvoiceRenderer:
    ...
    def generate(self, invoice) -> Tuple[str, str, str]:
        """
...
class BaseReportlabInvoiceRenderer(BaseInvoiceRenderer):
    ...
    def _get_stylesheet(self):
        ...
        stylesheet.add(ParagraphStyle(
            name='InvoiceFrom', parent=stylesheet['Normal']))
        stylesheet.add(ParagraphStyle(
            name='Small', fontSize=7, parent=stylesheet['Normal']))
        stylesheet.add(ParagraphStyle(name='Heading1',
                                      fontName=self.font_bold, fontSize=15, leading=15 * 1.2))
        stylesheet.add(ParagraphStyle(name='FineprintHeading',
                                      fontName=self.font_bold, fontSize=8, leading=12))
        stylesheet.add(ParagraphStyle(name='Fineprint',
                                      fontName=self.font_regular, fontSize=8, leading=10))
        return stylesheet
...
class ClassicInvoiceRenderer(BaseReportlabInvoiceRenderer):
    ...
    def _draw_invoice_to(self, canvas):
        ...
        p.drawOn(canvas, self.invoice_to_left,
    ...
    def _draw_invoice_from(self, canvas):
        ...
        p.drawOn(canvas, self.invoice_from_left,
    ...
    def _draw_logo(self, canvas):
        if self.invoice.contract.creditor.invoice_logo_image:
            logo_file = self.invoice.contract.creditor.invoice_logo_image
            ...
            canvas.drawImage(ir,
                             self.logo_left,
                             self.pagesize[1] -
                             self.logo_height - self.logo_top,
    ...
    def _draw_metadata(self, canvas):
        ...
        textobject.setFont(self.font_bold, 8)
        textobject.textLine(self._upper(pgettext('invoice', 'Contract UUID')))
        textobject.moveCursor(0, 5)
        textobject.setFont(self.font_regular, 10)
        textobject.textLine(str(self.invoice.contract.uuid))
        canvas.drawText(textobject)

        textobject = canvas.beginText(125 * mm, (297 - 50) * mm)
        textobject.setFont(self.font_bold, 8)
        if self.invoice.is_cancellation:
            textobject.textLine(self._upper(
                pgettext('invoice', 'Cancellation number')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            ...
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._upper(
                pgettext('invoice', 'Original invoice')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self.invoice.refers.number)
        else:
            textobject.textLine(self._upper(
                pgettext('invoice', 'Invoice number')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self.invoice.number)
        textobject.moveCursor(0, 5)
        ...
        if self.invoice.is_cancellation:
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._upper(
                pgettext('invoice', 'Cancellation date')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            ...
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._upper(
                pgettext('invoice', 'Original invoice date')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(date_format(
                self.invoice.refers.date, "DATE_FORMAT"))
            textobject.moveCursor(0, 5)
        else:
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._upper(
                pgettext('invoice', 'Invoice date')))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(date_format(self.invoice.date, "DATE_FORMAT"))
            textobject.moveCursor(0, 5)
    ...
    def _on_first_page(self, canvas: Canvas, doc):
        ...
        canvas.setTitle(pgettext('invoice', 'Invoice {num}').format(
            num=self.invoice.number))
    ...
    def _get_intro(self):
        ...
        #         '{}: {}'.format(
        #             self.invoice.event.settings.invoice_address_custom_field, self.invoice.custom_field),
        ...
        if self.invoice.invoice_to_vat_id:
            story.append(Paragraph(
                ...
                bleach.clean(self.invoice.invoice_to_vat_id,
                             tags=[]).replace("\n", "<br />\n"),
                self.stylesheet['Normal']
            ))
        ...
        if self.invoice.invoice_to_beneficiary:
            story.append(Paragraph(
                ...
                bleach.clean(self.invoice.invoice_to_beneficiary,
                             tags=[]).replace("\n", "<br />\n"),
                self.stylesheet['Normal']
            ))

        if self.invoice.introductory_text:
            story.append(Paragraph(self.invoice.introductory_text,
                                   self.stylesheet['Normal']))
    ...
    def _get_story(self, doc):
        ...
        tdata = [(
            pgettext('invoice', 'Item'),
            pgettext('invoice', 'Start'),
            pgettext('invoice', 'End'),
            pgettext('invoice', 'Qty'),
            pgettext('invoice', 'Unit'),
            pgettext('invoice', 'Price'),
            pgettext('invoice', 'Net'),
            pgettext('invoice', 'Discount'),
            pgettext('invoice', 'Disc. Net'),
        ...
        total = Decimal('0.00')
        currency = ''
        for line in self.invoice.positions.all():
            currency = line.currency
            tdata.append((
                Paragraph(line.item, self.stylesheet['Small']),
                Paragraph(line.period_start.strftime(
                    '%Y-%m-%d %H:%m'), self.stylesheet['Small']),
                Paragraph(str(line.period_end.strftime(
                    '%Y-%m-%d %H:%m')), self.stylesheet['Small']),
                normalize_fraction(line.quantity),
                line.unit,
                money_filter(line.price, currency),
                money_filter(line.net, currency),
                line.discount,
                money_filter(line.discounted_net, currency),
                '{}%'.format(normalize_fraction(line.tax_rate)),
                money_filter(line.gross, currency),
            ))
            taxvalue_map[line.tax_rate] += line.discounted_net
            grossvalue_map[line.tax_rate] += line.gross
            total += line.gross

        tdata.append([
            '', '', '', '', '', '', '', pgettext(
                'invoice', 'Invoice total'), '', '', money_filter(total, currency)
        ])
        colwidths=[
            a * doc.width for a in (.20, .12, .12, .07, .07, .09, .09, .09, .09, .09)]
        ...
        if self.invoice.additional_text:
            story.append(Paragraph(self.invoice.additional_text,
                                   self.stylesheet['Normal']))
            story.append(Spacer(1, 15 * mm))
        ...
        for rate, gross in grossvalue_map.items():
            if rate == 0:
                continue
            net=taxvalue_map[rate]
            tdata.append([
                localize(rate) + " % ",
                money_filter(net, currency),
                money_filter(gross, currency),
                money_filter(gross - net, currency),
            ])

        if len(tdata) > 1:
            colwidths=[a * doc.width for a in (.25, .15, .15, .15, .3)]
            table=Table(tdata, colWidths=colwidths,
                          repeatRows=2, hAlign=TA_LEFT)
            table.setStyle(TableStyle(tstyledata))
...
class InvoiceRenderer(ClassicInvoiceRenderer):
    identifier='standard'
    verbose_name=gettext_lazy('Invoice Renderer')
    ...
    def _draw_metadata(self, canvas):
        ...
        textobject=canvas.beginText(
            self.left_margin, self.pagesize[1] - begin_top)
        textobject.setFont(self.font_regular, 8)
        textobject.textLine(pgettext('invoice', 'Contract UUID'))
        textobject.moveCursor(0, 5)
        textobject.setFont(self.font_regular, 10)
        textobject.textLine(str(self.invoice.contract.uuid))
        canvas.drawText(textobject)
        ...
        p=Paragraph(date_format(self.invoice.date, "DATE_FORMAT"),
                      style=self.stylesheet['Normal'])
